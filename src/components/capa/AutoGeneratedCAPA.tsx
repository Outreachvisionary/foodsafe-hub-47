...
const AutoGeneratedCAPA: React.FC<AutoGeneratedCAPAProps> = ({
  ...
  const getPriorityFromSeverity = (severity?: string): CAPAPriority => {
    if (!severity) return CAPAPriority.Medium;
  ...
  const getSourceIcon = (source: CAPASource) => {
    switch (source) {
      ...
      case 'haccp' as CAPASource:
        return <ShieldAlert className="h-5 w-5 text-red-600" />;
      case CAPASource.CustomerComplaint:
        return <MessageCircle className="h-5 w-5 text-amber-600" />;
      case CAPASource.SupplierIssue:
        return <AlertTriangle className="h-5 w-5 text-purple-600" />;
      case 'traceability' as CAPASource:
