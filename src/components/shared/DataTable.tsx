...
const DataTable: React.FC<DataTableProps> = ({
  ...
  return <div className="overflow-hidden">
      {/* Header */}
      <div className="grid gap-4 p-4 bg-muted/30 border-b border-border font-medium text-sm text-muted-foreground" style={{
      ...
      <div className="divide-y divide-border">
        {data.map((item, index) => <div key={item.id || index} className="grid gap-4 p-4 hover:bg-muted/30 transition-colors" style={{
        gridTemplateColumns: `repeat(${columns.length}, 1fr) auto`
      }}>
            {columns.map(column => <div key={column.key} className="text-sm">
